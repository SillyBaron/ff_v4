...
def get_names_and_description():
    with open(CLUSTER_NAME_AND_DESCRIPTIONS, 'r') as f:
...
cluster_name_and_descriptions=get_names_and_description()



predicted_cluster_id=predict_model(model,person_df)['Cluster'].values[0]
predicted_cluster_data=cluster_name_and_descriptions[predicted_cluster_id]

st.header(f'Najbliżej Ci do {predicted_cluster_data["name"]}')
...
fig.update_layout(
    title="Rozkład wykształcenia w gurpie",
...
fig.update_layout(
    title="Rozkład ulubionych zwierząt w gurpie",
    xaxis_title='Ulubione zwierzę',
    yaxis_title='Liczba osób',
)
st.plotly_chart(fig)
...
fig.update_layout(
    title="Rozkład ulubionych miejsc w gurpie",
...
fig.update_layout(
    title="Rozkład na płcie w gurpie",
    xaxis_title='Płeć',
    yaxis_title='Liczba osób',
)
st.plotly_chart(fig)